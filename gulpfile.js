...
function doHBS(path, done) {
    pump(
        [src([`${path}/*.hbs`, `${path}/partials/**/*.hbs`]), livereload()],
        handleError(done)
    );
}
...
function doCSS(path, done) {
    pump(
        [
            src(`${path}/assets/css/screen.{scss,css}`, { sourcemaps: true })
                .pipe(sass({ outputStyle: "compressed" }))
                .pipe(postcss([easyimport, autoprefixer(), cssnano()]))
                .pipe(dest(`${path}/assets/built/`, { sourcemaps: "." })),
            livereload(),
        ],
        handleError(done)
    );
...
function main(done) {
    const tasks = glob
        ...
        .map((path) => {
            ...
            function package(taskDone) {
                ...
                const hbsWatcher = () =>
                    watch([`${path}/*.hbs`, `${path}/partials/**/*.hbs`], hbs);
                hbsWatcher.displayName = `hbsWatcher_${packageName}`;

                const cssWatcher = () =>
                    watch(`${path}/assets/css/**/*.{scss,css}`, css);
    ...
    const sharedCSSWatcher = () =>
        watch("packages/_shared/assets/css/**/*.css", sharedCSS);
    ...
    const sharedJSWatcher = () =>
        watch("packages/_shared/assets/js/**/*.js", sharedJS);
...
function doLint(theme, fix, done) {
    ...
    if (theme) {
        source = [
            `packages/${theme}/assets/css/**/*.css`,
            "packages/_shared/assets/css/**/*.css",
        ];
    }
...
function symlink(done) {
    ...
    exec(
        `ln -sfn ${__dirname}/packages/${argv.theme} ${argv.site}/content/themes`
    );
    done();
}
...
function test(done) {
    ...
    const testGScan = (gscanDone) => {
        glob.sync("packages/*", { ignore: "packages/_shared" }).forEach(
            (path) => {
...
function testCI(done) {
    ...
    const testGScan = (gscanDone) => {
        exec(
            `gscan --fatal --verbose packages/${argv.theme} --colors`,
...
function zipper(done) {
    ...
    const filename =
        require(`./packages/${argv.theme}/package.json`).name + ".zip";
